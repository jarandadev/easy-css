...
export const TagEditorHTML = ({ inputHTML }: Props) => {
  ...
  return (
    <div className=" bg-blue-500 rounded-md border px-2 py-1 flex gap-6 justify-between">
      <p className="text-white font-bold">{inputHTML.element.name}</p>
      <div className="flex">
        <button onClick={() => setOpen(true)}>
          <Icon.Edit className="hover:text-black" />
        </button>
        <button onClick={() => deleteInputHTML(inputHTML)}>
          <Icon.Delete className="hover:text-black" />
        </button>
      </div>
